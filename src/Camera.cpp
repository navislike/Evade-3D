...
void CCamera::SetViewPoint(glm::vec3 &viewpoint){
	m_view = viewpoint;
}
...
void CCamera::TranslateByKeyboard(double dt)
{
	...
	if (GetKeyState(VK_LEFT) & 1){
		if (isMiddle){
			switchPathPosition(true, false, false);
		}
		else if (isRightSide){
			switchPathPosition(false, true, false);
		}
		else if (isLeftSide){
			switchPathPosition(true, false, false);
		}
	...
	if (GetKeyState(VK_RIGHT) & 1){
		if (isMiddle){
			switchPathPosition(false, false, true);
		}
		else if (isLeftSide){
			switchPathPosition(false, true, false);
		}
...
void CCamera::switchPathPosition(bool left, bool mid, bool right){
	isLeftSide = left;
	isMiddle = mid;
	isRightSide = right;
}

bool CCamera::getIsMiddle(){
	return isMiddle;
}

bool CCamera::getIsLeftSide(){
	return isLeftSide;
}

bool CCamera::getIsRightSide(){
	return isRightSide;
}

void CCamera::setIsLeftSide(const bool left){
	isLeftSide = left;
}
void CCamera::setIsRightSide(const bool right){
	isRightSide = right;
}
void CCamera::setIsMiddle(const bool mid){
	isMiddle = mid;
}
